...
- **Backend specification** requires an absolute URL with scheme and a metrics poll interval; defaults are `http://localhost:8000` and `1s`. It also declares `kv_unified` (default `false`), which mirrors llama-server's `-kvu` flag and selects the `/slots` pressure denominator for llama.cpp backends (ignored for vLLM). Slot pinning is automatic — the slot count is read from the `/slots` endpoint (the same scrape that derives KV pressure), so there is no `id_slot` configuration key; see [[gateway#Session Slot Pinning]].
