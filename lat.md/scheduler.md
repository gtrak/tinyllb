...
- Only admits a request flow as long as the total number of active flows stays below the effective ceiling: the configured `max_active_flows`, dynamically lowered by the KV-pressure concurrency cap under backend KV pressure (see [[scheduler_policies#KV-Pressure Concurrency Cap]]).
...
**Permit pool and effective cap.** The permit pool is a static budget: the shared DRR state holds `max_permits` (initialized to `max_active_flows`) alongside `available_permits`, so the active-flow count is derivable as `max_permits - available_permits`. The background admission loop grants permits only while the active count is below the effective cap — the KV-pressure concurrency cap, equal to `max_active_flows` when that cap is disabled (see [[scheduler_policies#KV-Pressure Concurrency Cap]]). The loop wakes on the completion notify **or** a backend-monitor snapshot change (via `BackendMonitor::snapshot_receiver()`), so a KV-pressure drop reopens admissions within one `metrics_interval` without a completion; when the snapshot channel has no live sender (e.g. an empty monitor in tests) the loop falls back to notify-only waits.
...
**Permit accounting.** The sum of active flows and available permits equals the static permit budget (stored as `max_permits`, initialized to the configured maximum); permits are never created or destroyed beyond this bound, and the active count is derivable as `max_permits - available_permits`. New permits are granted only while the active count is below the *effective cap* — the KV-pressure concurrency cap ([[scheduler_policies#KV-Pressure Concurrency Cap]]), which equals `max_active_flows` when that cap is disabled. The same `active < cap` check guards the `try_select` early return, bounding the starvation force-admit path as well. Active flows may transiently exceed a lowered cap and drain to it as permits are released; in-flight flows are never aborted.
...
- [[scheduler_policies#KV-Pressure Concurrency Cap]] — Dynamic effective cap on permit grants (pressure-capped)
