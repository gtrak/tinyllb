# Spec: [[c_telemetry]] — Telemetry

## Purpose

Telemetry initializes the global tracing subscriber so that all structured log events and span data produced by the proxy are collected, filtered, and emitted in a consistent format. It provides a single bootstrap surface that the application calls once at startup before any diagnostic output is generated.
...
- **Filter directive** — the `RUST_LOG` environment variable controls per-module verbosity. Defaults to `info,llm_qdisc_proxy=debug` when absent. If `RUST_LOG` is present but empty, the empty string is passed to the tracing system as the filter directive rather than falling back to the default.
...
- When `RUST_LOG` is absent, the filter directive resolves to `info,llm_qdisc_proxy=debug`. When `RUST_LOG` is present but empty, the empty string is used as the filter — distinct from the absent case.
...
- [[?c_metrics_registry]] — separate concern: metrics collection is not part of telemetry initialization
- [[?c_gateway_proxy]] — the proxy application invokes telemetry initialization at startup before serving requests
